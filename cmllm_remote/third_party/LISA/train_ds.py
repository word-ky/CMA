...
def parse_args(args):
    ...
    parser.add_argument("--mr_ref_data", default="", type=str)
...
def main(args):
    ...
    if os.environ.get("RESET_REF_INPUT_PROJECTION", "0") == "1":
        ref_input_fcs = model.get_model().ref_input_fcs
        torch.nn.init.zeros_(ref_input_fcs[-1].weight)
        torch.nn.init.zeros_(ref_input_fcs[-1].bias)
        if args.local_rank == 0:
            print("Reset ref_input_fcs final projection to zero.")

    # make text_hidden_fcs, ref modules, mask_decoder, lm_head, embed_tokens trainable
...
def train(
    ...
    ref_mask_losses = AverageMeter("RefMaskLoss", ":.4f")
    rank_losses = AverageMeter("RankLoss", ":.4f")
    ...
    for global_step in range(args.steps_per_epoch):
        for i in range(args.grad_accumulation_steps):
            ...
            ref_mask_loss = output_dict.get("ref_mask_loss", mask_loss.new_tensor(0.0))
            rank_loss = output_dict.get(
            ...
            ref_mask_losses.update(ref_mask_loss.item(), input_dict["images"].size(0))
            rank_losses.update(rank_loss.item(), input_dict["images"].size(0))
            model.backward(loss)
            model.step()
        ...
        if global_step % args.print_freq == 0:
            if args.distributed:
                ...
                ref_mask_losses.all_reduce()
                rank_losses.all_reduce()
            ...
            if args.local_rank == 0:
                ...
                writer.add_scalar("train/ref_mask_loss", ref_mask_losses.avg, global_step)
                writer.add_scalar("train/rank_loss", rank_losses.avg, global_step)
                writer.add_scalar(
