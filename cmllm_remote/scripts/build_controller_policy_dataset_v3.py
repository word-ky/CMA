...
free-form JSON action names caused schema drift. Policy v2 fixed schema drift
with a single integer action index, but rollout showed that DPO can still learn
premature stop, invalid precondition calls, and no-op loops. This v3 builder
keeps the strict action-index schema and adds state-validity / stop-calibration
...
def preference_pair(pair_id, ep, observation, chosen_id, rejected_id, source, margin=1.0, meta=None):
    ...
    return {
        ...
        "episode_id": ep.get("episode_id"),
        "schema": "controller_policy_preference_v3_action_index",
        "source": source,
...
def add_pair(rows, pair_id, ep, observation, chosen_id, rejected_id, source, margin=1.0, meta=None):
    if chosen_id == rejected_id:
        return
    if chosen_id not in ACTION_TO_INDEX or rejected_id not in ACTION_TO_INDEX:
        return
    rows.append(preference_pair(pair_id, ep, observation, chosen_id, rejected_id, source, margin, meta))


def previously_accepted(prev_steps):
    for step in prev_steps:
        if step.get("decision") == "accept" and step.get("action") in SEGMENT_ACTIONS:
            return True
    return False


def invalid_or_unsafe_rejections(observation):
    """Actions that are unsafe under observable tool-precondition flags."""

    flags = observation.get("state_flags", {})
    rejects = []
    if not flags.get("has_target_candidate"):
        rejects.extend(
            [
    ...
    if not flags.get("has_anchor_ref"):
        rejects.extend(
            [
    ...
    if not flags.get("has_blackout_state"):
        rejects.extend(["SEG_TARGET_WITH_REF", "LOCAL_ENHANCE_REF_TARGET", "SEG_REF_TARGET_LOCAL"])
    if not flags.get("has_local_target_state"):
        rejects.extend(["SEG_TARGET_LOCAL", "SEG_REF_TARGET_LOCAL"])
    if not flags.get("has_rollback_available"):
        rejects.extend(["ROLLBACK_LOCAL_RESULT", "ROLLBACK_TO_BEST_DIRECT"])

    seen = set()
    ordered = []
    for action_id in rejects:
...
def make_regularization_preference_rows(ep, trace_source):
    """Add v3 pairs for state validity, stop calibration, and failed-action loops."""

    rows = []
    steps = ep.get("steps", [])
    for i, step in enumerate(steps):
        try:
            chosen_id = canonical_action(step)
        except KeyError:
            continue
        ...
        if chosen_id != "STOP_SUCCESS" and not previously_accepted(prev_steps):
            add_pair(
                rows,
                f"stop_calibration_{step.get('step')}",
                ep,
                observation,
                chosen_id,
                "STOP_SUCCESS",
                f"{trace_source}:v3_stop_calibration",
                margin=1.0,
                meta={"chosen_source_action": step.get("action")},
            )

        for rejected_id in invalid_or_unsafe_rejections(observation)[:4]:
            add_pair(
                rows,
                f"state_validity_{step.get('step')}_{rejected_id}",
                ep,
                observation,
                chosen_id,
                rejected_id,
                f"{trace_source}:v3_state_validity",
                margin=1.0,
                meta={"chosen_source_action": step.get("action")},
            )
        ...
        if prev_steps:
            prev = prev_steps[-1]
            try:
                prev_id = canonical_action(prev)
            except KeyError:
                prev_id = None
            if (
                prev_id
                and prev_id != chosen_id
                and prev.get("decision") in {"failed", "skip", "rollback", "invalid"}
            ):
                add_pair(
                    rows,
                    f"avoid_repeat_failed_{step.get('step')}_{prev_id}",
                    ep,
                    observation,
                    chosen_id,
                    prev_id,
                    f"{trace_source}:v3_avoid_repeat_failed",
                    margin=0.5,
                    meta={"previous_decision": prev.get("decision")},
                )
    return rows
...
def make_preference_rows(ep, trace_source):
    ...
    rows.extend(make_regularization_preference_rows(ep, trace_source))
    return rows
...
def main():
    ...
    pref_rows = []
    stats = {
        "schema": "controller_policy_dataset_v3_action_index",
    ...
    write_jsonl(out_dir / "controller_policy_trajectories_v3.jsonl", trajectories)
    write_jsonl(out_dir / "policy_sft_all_v3.jsonl", sft_rows)
    write_jsonl(out_dir / "policy_sft_train_v3.jsonl", sft_train)
    write_jsonl(out_dir / "policy_sft_val_v3.jsonl", sft_val)
    write_jsonl(out_dir / "action_preference_pairs_all_v3.jsonl", pref_rows)
    write_jsonl(out_dir / "action_preference_pairs_train_v3.jsonl", pref_train)
    write_jsonl(out_dir / "action_preference_pairs_val_v3.jsonl", pref_val)
    ...
    (out_dir / "controller_policy_dataset_stats_v3.json").write_text(
        json.dumps(stats, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    report = [
        "# Controller Policy Dataset v3\n\n",
        "Assistant outputs are strict `{\"action_index\": int}` JSON. v3 keeps the v2 action schema and adds DPO regularization pairs for state validity, stop calibration, and failed-action loops.\n\n",
