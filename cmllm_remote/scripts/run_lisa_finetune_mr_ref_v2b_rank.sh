...
BASE_VERSION="${BASE_VERSION:-/home/wjq/cmllm/outputs/lisa_finetune/cmllm_lisa_plus_mr_ref_v2_segllm_focus_nanguard_pilot500_20260515_114452/merged_hf}"
...
EXP="${EXP_NAME:-cmllm_lisa_plus_mr_ref_v2b_rank_$(date +%Y%m%d_%H%M%S)}"
OUT_ROOT="${OUT_ROOT:-/home/wjq/cmllm/outputs/lisa_finetune}"
LOG="${LOG:-/home/wjq/cmllm/outputs/lisa_finetune_mr_ref_v2b_rank.log}"
ENV_OUT="${ENV_OUT:-/home/wjq/cmllm/outputs/lisa_finetune_mr_ref_v2b_rank_current_run.env}"
EPOCHS="${EPOCHS:-1}"
...
COUNTERFACTUAL_RANK_LOSS_WEIGHT="${COUNTERFACTUAL_RANK_LOSS_WEIGHT:-0.5}"
COUNTERFACTUAL_RANK_MARGIN="${COUNTERFACTUAL_RANK_MARGIN:-0.05}"
...
COUNTERFACTUAL_RANK_LOSS_WEIGHT=$COUNTERFACTUAL_RANK_LOSS_WEIGHT
COUNTERFACTUAL_RANK_MARGIN=$COUNTERFACTUAL_RANK_MARGIN
...
if [[ "$SKIP_MERGE" != "1" ]]; then
  /home/wjq/venvs/rex/bin/python merge_lora_weights_and_save_hf_model.py \
    ...
    --sam_prompt_embed_clamp="$SAM_PROMPT_EMBED_CLAMP" \
    --counterfactual_rank_loss_weight="$COUNTERFACTUAL_RANK_LOSS_WEIGHT" \
    --counterfactual_rank_margin="$COUNTERFACTUAL_RANK_MARGIN" 2>&1 | tee -a "$LOG"
else
  MERGED="SKIPPED"
fi

cat > "$OUT_DIR/report.md" <<EOF
# MR-LISA++ v2B Counterfactual Rank Fine-Tune Report
...
- Counterfactual rank loss weight: $COUNTERFACTUAL_RANK_LOSS_WEIGHT
- Counterfactual rank margin: $COUNTERFACTUAL_RANK_MARGIN
