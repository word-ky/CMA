...
SELECTION_SHA='de4335243a9188cf0e2b55046668eca338fc85ec0e8a5b3c609d490f07e8503f'
...
def main(root,stage):
 root=Path(root);work=root/'research_log/cycle025';work.mkdir(exist_ok=True)
 frozen=root/'research_log/cycle024/documented_protocol_disjoint_candidates.jsonl'
 assert sha(frozen)==SELECTION_SHA
 selected=rows(frozen);assert len(selected)==50
 exclusion_path=root/'research_log/cycle024/exclusion_registry.json'
 assert sha(exclusion_path)=='837088966a5ef3fc6f24515318b4e4f0743a9c76e54a08fa1fcd8cbc17768aa7'
 ...
 qc={r['counterfactual_id']:r for r in json.loads((root/'research_log/cycle024/selected_group_qc_provenance.json').read_text())}
 if stage=='pre':
  audit=[];valid=[]
  ...
  if len(valid)<40:raise SystemExit('STOP_ASSET_VALIDITY_BELOW40')
 else:
  pre=json.loads((work/'pre_restoration_audit.json').read_text());data=root/'shared/data/cycle025'
  ...
  for record in pre['groups']:
   ...
   if not reasons:
    try:
     ...
     w,h=Image.open(restored_g['image_path']).size;miner_hashes=[]
  ...
  print('ASSET_AUDIT_FROZEN',len(valid),50-len(valid),flush=True)
  if len(valid)<40:raise SystemExit('STOP_ASSET_VALIDITY_BELOW40')
