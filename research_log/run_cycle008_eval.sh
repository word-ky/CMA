...
if [[ "$SPLIT" == val ]]; then
  ...
  GROUPS_FILE="$ROOT/shared/data/cycle003/counterfactual_holdout_first30_restored.jsonl"
  PAIRS_FILE="$ROOT/shared/data/cycle003/helmet_miner_pairs_restored.jsonl"
fi
EXTRA=()
if [[ "$MODEL_KIND" == adapted ]]; then EXTRA=(--adaptation-checkpoint "$ROOT/outputs/cycle008/train/last.pt"); fi
for COND in clean target15_b; do
  OUT="$ROOT/outputs/cycle008/${SPLIT}_${MODEL_KIND}_$COND"
