...
"$PY" scripts/eval_mr_ref_counterfactual_v0.py "${COMMON[@]}" --max-items 1 --out-dir "$ROOT/outputs/cycle010/full_regression"
"$PY" "$ROOT/research_log/cycle010_analysis.py" "$ROOT" regression
"$PY" scripts/eval_mr_ref_counterfactual_v0.py "${COMMON[@]}" --memory-reobservation --out-dir "$ROOT/outputs/cycle010/val_mgr_target15_b"
"$PY" scripts/eval_counterfactual_memory_fidelity.py --input "$ROOT/outputs/cycle010/val_mgr_target15_b/memory_predictions.jsonl" --output "$ROOT/outputs/cycle010/val_mgr_target15_b/memory_metrics.json" --min-iou 0.5
"$PY" "$ROOT/research_log/cycle010_analysis.py" "$ROOT" analyze
