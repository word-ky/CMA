...
def main():
    ...
    out = root / 'outputs/cycle036/crop_zero'
    ...
    control_path = work / 'cma_prediction_freeze.json'
    control = json.loads(control_path.read_text())
    for trial in control['trials']:
        assert sha(trial['prediction']) == trial['prediction_sha256']
    assert control['checkpoint_provenance'] == weight_hashes
    assert control['protocol_sha256'] == sha(work / 'protocol_receipt.json')
    assert sha(work / 'execution_manifest.json') == protocol['execution_manifest_sha256']
    for relative, digest in protocol['frozen_code_hashes'].items():
        assert sha(root / 'code/cmllm' / relative) == digest
    expected = json.loads((root / 'research_log/cycle036/source_binding.json').read_text())
    for relative, digest in expected.items():
        source_path = root / relative
        import hashlib
        assert hashlib.sha256(source_path.read_text().encode()).hexdigest() == digest, relative
    assert not (out / 'prediction_freeze.json').exists(), 'Already frozen: do not rerun'
    started = time.time()
    model, tokenizer, dtype, processor, transform = load_base_w15(root)
    from crop_zero_hook import install
    intervention = install(model)
    ...
    assert len(intervention['calls']) == 100
    freeze['intervention'] = intervention
    freeze['control_freeze_sha256'] = sha(control_path)
    freeze['source_binding'] = expected
    freeze['runner_sha256'] = sha(__file__)
    freeze['hook_sha256'] = sha(Path(__file__).with_name('crop_zero_hook.py'))
