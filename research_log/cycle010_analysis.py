"""Fixed MGR comparison; GT-oracle choices here are analysis only."""
import hashlib
import json
...
out=root/'outputs/cycle010'
log=root/'research_log/cycle010';log.mkdir(exist_ok=True)
...
if sys.argv[2]=='regression':
    ...
    mgr=out/'val_mgr_target15_b'
    check_pairs(base,mgr)
    a,b=read(base/'memory_metrics.json'),read(mgr/'memory_metrics.json')
    ...
    for x,y in zip(a['groups'],b['groups']):
        ...
        use_mgr=(y['cmsa'],y['target_miou'])>(x['cmsa'],x['target_miou'])
        oracle.append(y if use_mgr else x)
        miou_oracle.append(y if y['target_miou']>x['target_miou'] else x)
        choices.append({'group_id':x['group_id'],'choice':'MGR' if use_mgr else 'full','cmsa_full':x['cmsa'],'cmsa_mgr':y['cmsa'],'miou_full':x['target_miou'],'miou_mgr':y['target_miou']})
    ...
    result={'status':'PASS' if all(criteria.values()) else 'FAIL','criteria':criteria,
            'full':a['summary'],'mgr':b['summary'],'mgr_minus_full':delta,'cmsa_transitions':changes,
            ...
            'paired_original_inputs_and_masks_equal':True,'new_training_runs':0,'mgr_validation_runs':1,'scale':1.25}
    (log/'comparison.json').write_text(json.dumps(result,indent=2)+'\n')
    for folder in (out/'full_regression',mgr):
    ...
    hashes={str(p.relative_to(root)):hashlib.sha256(p.read_bytes()).hexdigest() for p in
            (root/'code/cmllm/scripts/memory_reobservation.py',root/'code/cmllm/scripts/eval_mr_ref_counterfactual_v0.py',root/'research_log/run_cycle010.sh')}
