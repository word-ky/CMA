# Cycle010 — fixed Memory-Guided Re-observation

2026-09-18 07:40 +08:00: synced869cded and mirrored review009 verbatim. No training. Reuse Cycle008 base validation D-full metrics, same base w15/evaluator/scorer and frozen validation50. Do not touch diagnostic/confirmation.

Reuse map: build_item owns seeded full-frame degradation, supplied REF encoding and model tensors; extend only optional one-memory ROI transform. Existing ResizeLongestSide1024 + bottom/right SAM padding preserves aspect ratio; no new interpolation/resolution rule. Export original-coordinate prediction/targets/references through existing exporter and unchanged CMF scorer. New pure geometry helper only needs supplied miner bbox and image dimensions; no helmet input. D-MGR runs separately per identity because ROI differs; weights/prompt unchanged.

Increment1: bbox1.25 center expansion, floor left/top and ceil right/bottom, image clipping; crop reference mask and shift bbox; inverse paste with outside zero. Test geometry,borders,no target input before evaluator integration. Increment2: integrate optional path, check all tests and one real full validation run. Fixed gate: mIoU+0.02,CMSA nondecrease,>=3 fail-to-pass,<=1 pass-to-fail. No scale tuning.

Oracle analysis predefined: per-group select lexicographically by GT CMSA then GT group mIoU (ties full); recompute aggregate metrics for chosen groups. This upper-bounds CMSA union, not every supporting metric. Also report separate mean max group mIoU as mIoU-specific oracle upper bound. Neither is runtime selection.

07:43 +08:00: geometry+integration passed32 tests. Integration changes helmet-label pixels and confirms all model tensors unchanged; original-coordinate labels retained only for offline scoring. Test-only Windows Unicode OpenCV path failure repaired by relative fixture paths; no model patch needed. Before MGR evaluation, one full-frame first-group regression must match saved base predictions exactly.


07:44 +08:00: run20260918-074352-cma-cycle010-mgr launched. NVML read failed as in prior cycles; torch CUDA available,50,598,707,200 bytes free GPU0. No driver changes. First full-frame regression then one MGR validation50 run.
